...
/** 
    * Extended Tic Tac Toe Game with Multiple Difficulty Levels and Score Tracking
    * Player vs Computer, choose difficulty, marker and view stats as you play
...
void easy(gamestats &stats){
    ...
    //the user can decide to restart from here.
...
void normal(gamestats &stats){
    ...
    //Select marker for player and computer
    std::cout<<"\n";
    std::cout<<"_____CHARACTERS_____\n";
    std::cout<<"Choose your marker (X or O):";
    while(true){
        std::cin >> player;
        player = toupper(player); //Convert to uppercase in case user enters lowercase 
        if(player == 'X' || player == 'O'){
            computer = (player == 'X') ? 'O' : 'X';
            break;
        }
    ...
    //Displaying how the board is designed with each cell's number on it before starting the game
    ...
    while(gameRunning){
        ...
        computermove_normal(spaces,computer, player); 
        gameboard(spaces);
